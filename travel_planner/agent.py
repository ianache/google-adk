...
coordinator_agent = Agent(
        ...
        instruction = f"""
          ...
          You will receive user input in natural langyage and need to extract the following details:

          Note that maximum budge should be used for both flight and hotel suggestions, it should not be exceeded when combined.
          if the user doesn't specify a start_date , but something like "Next week","nex month", or "Next year", convert it to a dae format by taking today's datte as {today}.
          ...
          - If not start_date is provided, ask the user to specify a preferred date or default to today's date.
          ...
          Step 5: Present the final results combining the results from agents and a day plan including:
          - Trip summary with all details (origin, destination, start date, end date, budget).
          - Note that maximunm budget should be used for flight and hotel suggestions. it should not be exceeded when combined.
          - Flight suggestions
          - Hotel suggestions
          - Total estimated cost
          - A suggested day plan for the trip, including activities and places to visit in the destination.

          Be concise, clear, and friendly in guiding the user. If you encouter any missing information , ask the user for clarification.
          """,
